...
public class Configuration {
    ...
    void postConstruct() {
        ...
        if (activeProfiles.contains(Profiles.TEST)) {
            return;
        }
        ...
        if (!missingConfigurationProperties.isEmpty()) {
            String errorMessage = "Invalid configuration:\n\n";
            errorMessage += "Missing configuration properties:\n";
            for (String missingConfigurationProperty : missingConfigurationProperties) {
                errorMessage += "- " + missingConfigurationProperty + "\n";
            }
            errorMessage += "\n";
            errorMessage += "Configuration variables can be set in several ways, for example:\n\n";

            errorMessage += "By using command line arguments when starting the application via Maven, e.g.:\n"
            ...
            errorMessage += "By using command line arguments when starting the application as a JAR file, e.g.:\n"
                    + "java -jar public-api-sample-alarm-follower-1.0.0-SNAPSHOT.jar --apiKey=AK1/abc/xyz "
                    + "--organizationId=697bc8c3-897a-472d-abd2-0a0ebd3f703a --apiUrl=https://api.ecostruxureit.com/rest/v1\n\n";

            errorMessage += "By setting properties in the application.properties file - e.g.:\n"
                    + "apiKey=AK1/abc/xyz\n"
                    + "organizationId=697bc8c3-897a-472d-abd2-0a0ebd3f703a\n"
                    + "apiUrl=https://api.ecostruxureit.com/rest/v1\n\n";

            errorMessage += "By setting environment variables - e.g.:\n"
                    + "API_KEY=AK1/abc/xyz\n"
